...
class Admin extends CI_Controller {
	...
	public function index()
	{
		...
		//code
		$transaksi_hari_ini = $this->mdl_dashboard->jumlah_transaksi_hari_ini();
		$jumlah_transaksi = 0;
		$jumlah_disc = 0;
		$barang_terjual =0;
		foreach ($transaksi_hari_ini->result() as $row) {
			$jumlah_transaksi += ($row->harga_jual-($row->harga_jual*$row->disc_1/100)-(($row->harga_jual-($row->harga_jual*$row->disc_1/100))*$row->disc_2/100)-(($row->harga_jual-($row->harga_jual*$row->disc_1/100)-(($row->harga_jual-($row->harga_jual*$row->disc_1/100))*$row->disc_2/100))*$row->disc_3/100)-$row->potongan)*$row->jumlah;
			$jumlah_disc += (($row->harga_jual*$row->disc_1/100)-(($row->harga_jual-($row->harga_jual*$row->disc_1/100))*$row->disc_2/100)-(($row->harga_jual-($row->harga_jual*$row->disc_1/100)-(($row->harga_jual-($row->harga_jual*$row->disc_1/100))*$row->disc_2/100))*$row->disc_3/100)-$row->potongan)*$row->jumlah;
			$barang_terjual += $row->jumlah;
		}
		$data['jumlah_transaksi'] = $jumlah_transaksi;
		$data['jumlah_disc'] = -$jumlah_disc;
		$data['barang_terjual'] = $barang_terjual;
		$data['kasir_transaksi'] = $this->mdl_dashboard->jumlah_kasir_hari_ini()->num_rows();
		$data['inventory'] = $this->mdl_dashboard->jumlah_barang();
		$data['uang_masuk'] = $this->mdl_dashboard->nominal_terjual();
		$data['laba'] = $this->mdl_dashboard->laba();
		$data['jumlah_terjual'] = $this->mdl_dashboard->terjual();
		$data['persentase_laba'] = $data['laba']/$data['uang_masuk']*100;
		$data['persentase_terjual'] = $data['jumlah_terjual']/($data['inventory']+$data['jumlah_terjual'])*100;
		//code
	...
	$data = $this->mdl_dashboard->laporan_jangka($tanggal_mulai, $tanggal_selesai);
	...
	public function do_upload(){
		...
		$this->upload->initialize($config);
         ...
         function update_barang(){
         	...
         	$data_barang = array(
         		...
         		'harga_minimum'  => $this->input->post('harga_minimum'),
		...
		// $kode_produk = $this->input->post('kode_produk');
         	...
         	$harga_minimum_edit = $this->input->post('harga_minimum_edit');
         	...
         	$object= array(
         		...
         		'harga_minimum'=>$harga_minimum_edit,
         		'nama_admin'=>$nama_admin,
         	);
         ...
         function posting_pembelian(){
         	...
         	foreach ($data_pembelian as $value) {
				...
				// $nama_admin = $nama_admin;
				// $tanggal = $tanggal
         		$this->db->query('UPDATE `produk` SET `harga` = '.$harga.', `jumlah` = jumlah + '.$jumlah.', `harga_jual` = '.$harga_jual.',`harga_minimum` = '.$harga_minimum.', `disc_1` = '.$disc_1.', `disc_2` = '.$disc_2.', `disc_3` = '.$disc_3.', `nama_admin` = "'.$nama_admin.'", `tanggal` = "'.$tanggal.'" WHERE `kode_produk` = '.$value->kode_produk.'');
         	}
