...
class Kasir extends CI_Controller {
	...
	public function rekap(){
		...
		$data['id_admin'] = $adm['id_admin'];
		$data['title'] = "Mitra Jaya-Kasir| $nama_admin";
		$data['konten'] = $this->load->view('kasir/konten/rekap',$data,true);
		$this->load->view('kasir/dashboard', $data);
	...
	function update_customer_info(){
		...
		$check = $this->db->get_where('customer_data', array('nama'=>$nama));
		if ($check->num_rows()==0){
			$this->db->where('customer_data_id', $id);
			$this->db->update('customer_info', array('no_hp'=>$no_hp));

			$this->db->where('id', $id);
			$query = $this->db->update('customer_data', array('nama'=>$nama, 'username'=>$username,));

			echo json_encode($query);
		}
	}
	function nota_get_cus(){
		$kode_faktur = $this->input->post('kode_faktur');
		if (isset($kode_faktur)) {
			$faktur = $kode_faktur;
			$querygetcustomer = $this->db->get_where('penjualan_barang', array('nomor_faktur'=>$faktur));
			foreach ($querygetcustomer->result() as $cus) {
				$customer_id = $cus->id_customer;
			}
			$this->db->select('* ,customer_data.id as id_customer, provinces.name as provinsi, regencies.name as kabupaten, districts.name as kecamatan');
	...
	function tambah_customer(){
		...
		$this->form_validation->set_rules('nama', 'Nama', 'required|is_unique[customer_data.nama]');
		if ($this->form_validation->run() == TRUE){
			$object_customer_data = array('nama' => $nama,'username'=>$email );
			$this->db->insert('customer_data', $object_customer_data);

			$this->db->order_by('id', 'desc');
			$this->db->limit(1);
			$cus_data = $this->db->get('customer_data');
			foreach ($cus_data->result() as $cus) {
				$id_customer = $cus->id;
			}
			$object_customer_info = array('customer_data_id' => $id_customer,'provinsi_id'=>$provinsi,'kabupaten_id'=>$kabupaten,'kecamatan_id'=>$kecamatan, 'kode_pos'=>$kode_pos, 'no_hp'=>$no_hp, 'alamat'=>$alamat );
			$insert_data = $this->db->insert('customer_info', $object_customer_info);
	...
	public function customer(){
		...
		$data['konten'] = $this->load->view('kasir/konten/customer',$data,true);
		$this->load->view('kasir/dashboard', $data);
	}
	function get_customer_tabel(){
		$query = $this->mdl_kasir->customer_data();
		echo json_encode($query->result());
	}
	function pilih_customer() {
		$result = $this->db->get("customer_data")->result();
	...
	function simpan_pembayaran(){
		...
		$uang = $this->input->post('uang_bayar');
		$potongan = $this->input->post('potongan');
		$kembalian = $this->input->post('kembalian');
		$kode = $this->input->post('kode_faktur');
		$data_penjualan = $this->db->get_where('daftar_penjualan', array('id_pembelian'=>$kode, 'status'=>0, 'status_bayar'=>0))->result();
		$adm = $this->_admin_data();
		$nama_admin = $adm['nama_admin'];
		$id_admin = $adm['id_admin'];
		...
		$this->db->update('daftar_penjualan', array('status'=>'1', 'status_bayar'=>'1' ,'tanggal_transaksi'=>$tanggal));
		$this->db->where('nomor_faktur', $kode);
		$query = $this->db->update('penjualan_barang', array('status'=>'1', 'tanggal_update'=>$tanggal, 'tanggal_posting'=>$tanggal, 'id_admin'=>$id_admin));
		$object_riwayat = array('faktur' =>$kode ,
			'tanggal_beli' =>$tanggal , 
			'harga' =>$total_beli , 
			'potongan' =>$potongan , 
			'jumlah_bayar' =>$total_beli-$potongan , 
			'jumlah_uang' =>$uang , 
			'jumlah_kembalian' =>$uang-$total_beli-$potongan , 
		);
		$this->db->insert('tanggal_beli', $object_riwayat);
		echo json_encode($query);
	}
	...
	function batal_faktur(){
		...
		$this->db->where('faktur', $kode);
		$this->db->delete('tanggal_beli');
	...
	function tambah_by_barcode(){
		...
		foreach ($query->result() as $produk) {
			...
			$harga_minimum = $produk->harga_minimum;
	...
	function get_faktur($id_admin){
		...
		$query_get = $this->db->get('penjualan_barang');
		$jumlah = $query->num_rows();
		if ($jumlah <= 0) {
			$faktur = $id_admin.date('Ymd').$jumlah;
		}else{
			foreach ($query_get->result() as $row) {
			$faktur = $row->nomor_faktur+1;
			}
		}
		// $faktur = $id_admin.date('Ymd').$jumlah;
